...
| **Inbyggd chatt-ruta** | ✅ Klart (CHAT-1, CHAT-2, P2-A, P2-B) |
| **Auto-dispatch till agent** | ✅ Klart (P2-C — startBuild() postar spec) |
| **GitHub push (pusha kod)** | ✅ Klart (P2-D — /api/github/push) |
| **GitHub inbound webhook** | ✅ Klart (app.py rad 4447–4482, verifierat iteration 7) |
| **Auto-callback efter bygge** | ⚠️ Delvis — webhook finns, agent-auto-callback saknas |
...
`_progress_set()` kör en O(n) GC-loop (tar bort entries äldre än 15 min) på varje anrop. Med 25 agenter per granskning och 3 simultana granskningar = 75 GC-loopar under `_progress_lock` per körning. Alla dessa håller `_progress_lock` i t